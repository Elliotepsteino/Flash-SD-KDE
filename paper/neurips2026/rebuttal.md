...
**Q3 (impact of TF32 precision):** TF32 rounds only the `tl.dot` *inputs* to a 10-bit mantissa; storage and accumulation remain FP32, and no data is cast to FP16. New experiment: $d=16$, $n_{\text{train}}=32{,}768$, $n_{\text{test}}=4{,}096$, against an FP64 reference of the full SD-KDE pipeline:
...
**Q3 (Laplace surrogate vs. full SD-KDE on non-Gaussian, heavy-tailed distributions):** New experiment: 1-D two-component Student-$t_3$ mixture (infinite fourth moment), Silverman bandwidth, ISE against the true density over 5 seeds ($\times 10^3$, mean $\pm$ std):
...
**W2 (baseline Tensor-Core usage; compiled graph of `torch.compile`):** In Table 1 the PyTorch baselines run at PyTorch's default matmul precision (TF32 disabled, i.e., FP32 CUDA-core GEMMs); we will state this explicitly. We have now rerun the eager baseline with TF32 *enabled*: 113.0 ms vs. 112.9 ms with TF32 off — no improvement. The profiler explains why: with TF32 on, cuBLAS does dispatch a Tensor-Core kernel (`cutlass_80_tensorop_s1688gemm`), but all GEMMs combined are only $11\%$ of the baseline's CUDA time; $\approx 89\%$ is materialized elementwise/reduction traffic (`vectorized_elementwise_kernel`, `reduce_kernel`). For `torch.compile` (TF32 on), the compiled score graph contains five kernels: two small norm/epilogue Triton kernels, one large fused subtract-clamp-`exp`-sum reduction ($50\%$ of kernel time), and two cuBLAS GEMMs ($XX^\top$ as TF32 tensorop, $25\%$; $\Phi X$ as `ampere_sgemm`, $25\%$). Inductor must *materialize* $\Phi$ — a 4.10 GB buffer, exactly one $32{,}768^2$ FP32 matrix — because a cuBLAS GEMM cannot consume a fused producer. Our kernel keeps that matrix in registers/shared memory (8.1 MB peak extra). Two further checks close off this line: a *manually tiled* eager implementation (row-chunked, never allocating more than tile $\times$ $n$; tile $\in \{512, \dots, 8192\}$) runs at 114–118 ms — tiling caps memory but not HBM traffic, since every tile still round-trips global memory between the cuBLAS call and the elementwise kernels — and `torch.compile(mode="max-autotune")`, which enables Inductor's Triton-GEMM templates with epilogue fusion, lands at 25.2 ms vs. 25.6 ms for default `torch.compile`. Both sit near the structural floor of PyTorch's execution model: with the Gram matrix and $\Phi$ each written and read once through HBM, the minimum traffic is $\approx 4 \times 4\,\text{GB} = 16$ GB $\approx 21$ ms at 770 GB/s — an order of magnitude above our 2.2 ms. Getting below that floor requires keeping the $n^2$ tile in registers across the GEMM$\to$`exp`$\to$accumulate chain, which is not expressible as a composition of PyTorch ops. We will include the full trace and this analysis in the revision.
