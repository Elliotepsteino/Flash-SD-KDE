...
|---|---|---|---|---|
| 1 | PyTorch eager, cuBLAS GEMMs + separate elementwise kernels (Table 1; unchanged with TF32 on) | (baseline) | 113.3 | $1\times$ |
...
**W2 (baseline Tensor-Core usage; compiled graph of `torch.compile`):** *Short answer: the eager baseline is bandwidth bound, with only $11\%$ of its runtime in GEMMs, so whether its GEMMs use Tensor Cores makes no meaningful difference.* The Table 1 baselines ran at PyTorch's default matmul precision (TF32 off, FP32 CUDA-core GEMMs), which we will state explicitly in the revision. When we enable TF32, cuBLAS does dispatch the Tensor-Core kernel `cutlass_80_tensorop_s1688gemm`, and the runtime is unchanged: 113.0 ms vs. 112.9 ms, matching Table 1's 113.3 ms. The profiler explains why: the GEMMs account for 12.8 ms of the 113 ms, so by Amdahl's law even free GEMMs would improve the baseline by at most $1.13\times$, against the $47\times$ gap to Flash-SD-KDE. The other $89\%$ of the time is spent moving $n^2$ intermediates through global memory, which no kernel-by-kernel implementation can avoid because separate kernels communicate only through it: writing and reading the Gram matrix and $\Phi$ (4 GB each) costs $\approx 16$ GB $\approx 21$ ms at 770 GB/s before any arithmetic, and tiling merely caps peak memory without reducing the bytes moved. Tensor Cores become useful once fusion removes the $n^2$ traffic entirely, at which point the same hardware yields $4.94\times$ inside our streamed kernel (Table 5). The ladder isolates each idea ($n_{\text{train}}=32{,}768$, $n_{\text{test}}=4{,}096$, $d=16$):
...
| PyTorch, FP32 (Table 1 baseline) | 100.8 | 11.9 | 112.8 | 89% |
| PyTorch, TF32 enabled | 100.8 | 12.1 | 112.8 | 89% |
