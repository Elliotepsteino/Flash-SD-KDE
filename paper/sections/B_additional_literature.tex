...
% [CHECKED FOR MISSING RESULTS: no experimental result was intended to appear in C.2.  This subsection is literature/future-work context; the compiled red sentence clarifies that compression methods change the estimator and are therefore not direct exact-baseline experiments.]
% \begin{revadded}
Compression methods change the statistical estimator by replacing the sample, whereas the experiments in this paper evaluate exact SD-KDE acceleration at fixed $n$; we therefore treat them as complementary future-work context rather than direct exact-runtime baselines.
% \end{revadded}
