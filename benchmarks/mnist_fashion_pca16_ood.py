...
from benchmarks.mnist_fashion_pca16_ood_config import MnistFashionOodConfig
from flash_sd_kde.config import select_emp_score_backend
...
def _emp_score_backend(
    ...
    *,
    ...
    backend = config.emp_score_backend
    if backend is None:
        backend = select_emp_score_backend(train.shape[0])

    if backend == EMP_SCORE_BACKEND_ORDERED_SPLITK:
    ...
    if backend == EMP_SCORE_BACKEND_SYMMETRIC_ATOMIC:
    ...
    raise ValueError(f"unsupported emp_score_backend: {backend}")
...
def run_benchmark(config: MnistFashionOodConfig) -> Path:
    ...
    for n_train in config.n_train_list:
        ...
        _ = _emp_score_backend(config, train_subset, h, device=device)
        torch.cuda.synchronize(device)
        ...
        def score_kernel():
            return _emp_score_backend(config, train_subset, h, device=device)
        ...
        results["metrics"][str(n_train)] = {
            "kde": metrics_kde,
            "emp_sd_kde": metrics_emp,
        }
        results["runtime_sec"][str(n_train)] = {
            "kde": runtime_kde,
    ...
    if config.save_density_arrays and densities_for_curves:
        np.savez_compressed(run_dir / "densities_curves.npz", **densities_for_curves)
