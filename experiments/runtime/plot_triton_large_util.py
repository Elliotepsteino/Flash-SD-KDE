...
def plot_util(rows: List[Dict[str, float]], output: Path):
    ...
    bars = plt.bar(idx, util, width, color="#f28e2b", label="Flash-SD-KDE (Ours)")
    ...
    plt.xlabel("$n_{\\text{train}}$ (samples, power of two)", fontsize=12)
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("util_1d_triton_scaling.pdf"),
